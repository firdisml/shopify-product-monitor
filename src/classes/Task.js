        ...
        this.task = setInterval(async () => {
            ...
            try {
                ...
                if (err.response && (err.response.status === 430 || err.response.status === 429)) {
                    ...
                    Log.Error(`${this.sellerUrl} has an high level of protection from monitors`);
                    ...
                    Log.Error(`Timeout occurred, a node js script cannot manage a lot of requests in the same time (I raccomand max 10 sites for script's istance, to stay more safe), please start more that 1 monitor with splitted sites`);
                    clearInterval(this.task)
                }
